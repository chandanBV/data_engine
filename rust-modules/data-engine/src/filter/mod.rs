...
impl<'a> FilterEngine<'a> {
    ...
    pub fn execute(&self, config: &FilterConfig) -> Result<Vec<RecordBatch>, String> {
        ...
        for (_idx, batch) in self.data.iter().enumerate() {
            #[cfg(target_arch = "wasm32")]
            web_sys::console::log_1(
                &format!(
                    "Filter: Processing batch {} with {} rows",
                    _idx,
                    batch.num_rows()
                )
                .into(),
        ...
        web_sys::console::log_1(
            &format!(
                "Filter: Complete. {} batches remaining",
    ...
    fn create_filter_mask(
        ...
        let column = batch
            .column_by_name(&filter_spec.field)
    ...
    fn create_equals_mask(
        ...
        match column.data_type() {
            ...
            DataType::Float64 => {
                ...
                let target_value: f64 = value
                    .parse()
                    .map_err(|_| format!("Cannot parse '{}' as float", value))?;

                let mask: Vec<bool> = (0..float_array.len())
                    .map(|i| {
                        !float_array.is_null(i)
                            && (float_array.value(i) - target_value).abs() < f64::EPSILON
                    })
                    .collect();
            ...
            _ => Err(format!(
                "Equals filter not supported for data type: {:?}",
                column.data_type()
            )),
        }
    }

    fn create_contains_mask(
        &self,
        column: &ArrayRef,
        filter_spec: &FilterSpec,
    ) -> Result<BooleanArray, String> {
    ...
    fn create_range_mask(
        &self,
        column: &ArrayRef,
        filter_spec: &FilterSpec,
    ) -> Result<BooleanArray, String> {
        let min_value = filter_spec
            .min_value
            .as_ref()
            .ok_or_else(|| "Min value required for range filter".to_string())?;
        let max_value = filter_spec
            .max_value
            .as_ref()
    ...
    fn combine_masks_and(
        &self,
        mask1: &BooleanArray,
        mask2: &BooleanArray,
    ) -> Result<BooleanArray, String> {
        // Use vectorized AND operation - much faster!
        compute::and(mask1, mask2).map_err(|e| format!("Failed to combine masks: {}", e))
    }

    fn filter_batch_manually(
        &self,
        batch: &RecordBatch,
        mask: &BooleanArray,
    ) -> Result<RecordBatch, String> {
